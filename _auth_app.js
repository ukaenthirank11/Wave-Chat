(function () {
  ...
  const storageKey = "wavechat.existing.state.v3";
  const splashDelayMs = 2400;
  const readyDelayMs = 1500;
  const otpLength = 6;
  const otpAutoVerifyDelayMs = 1600;
  ...
  function init() {
    ...
    if (state.screen === "splash") {
      queueSplashAdvance();
    } else if (state.screen === "otp" && state.otpMode === "auto") {
      startOtpAutoVerification();
  ...
  function bindEvents() {
    ...
    elements.app.addEventListener("change", handleChange);
  ...
  function buildConversations() {
    ...
    data.contacts.forEach(function (contact) {
      result[contact.id] = clone(
        (data.conversationSeeds && data.conversationSeeds[contact.id]) || [
          {
  ...
  function handleInput(event) {
    ...
    if (target.id === "phoneNumberInput") {
      state.phoneNumber = target.value.replace(/\D/g, "").slice(0, getCurrentCountry().maxLength);
      render(focus);
      return;
    }
  ...
  function handleChange(event) {
    ...
    if (target.id === "profilePhotoCameraInput") {
      handleProfilePhotoSelection(target.files && target.files[0], "camera");
      target.value = "";
      return;
    }

    if (target.id === "profilePhotoUploadInput") {
      handleProfilePhotoSelection(target.files && target.files[0], "gallery");
      target.value = "";
    }
  }
  ...
  function handleAction(action, element) {
    switch (action) {
      ...
      case "continue-phone":
        if (!isPhoneValid()) {
          flashToast(getPhoneValidation().message);
          return;
        }
        requestOtp();
        return;
      case "enter-otp-manually":
        state.otpMode = "manual";
        state.otpStatus = "idle";
        state.otpError = "";
        stopOtpAutoVerification();
        render({ focusId: "otp-0" });
        return;
      case "resend-otp":
        requestOtp(true);
        return;
      ...
      case "save-profile":
        ...
        return;
      case "capture-profile-photo":
        openProfilePhotoInput("camera");
        return;
      case "upload-profile-photo":
        openProfilePhotoInput("gallery");
        return;
      case "continue-profile-photo":
        navigate("notifications");
        return;
      case "skip-profile-photo":
        navigate("notifications");
        return;
      ...
      case "skip-notifications":
        state.notificationPreference = "skipped";
        navigate("ready");
        return;
  ...
  function renderScreen() {
    switch (state.screen) {
      ...
      case "profilePhoto":
        return renderProfilePhotoScreen();
      case "notifications":
  ...
  function renderPhoneScreen() {
    ...
    return (
      ...
      "<strong>Country check</strong>" +
      "<span>" +
      escapeHtml(country.validationHint) +
      "</span>" +
      "</div>" +
      '<label class="phone-field' +
      (!validation.valid && state.phoneNumber ? " phone-field--invalid" : "") +
      '">' +
      ...
      escapeHtml(country.dialCode) +
      "</span>" +
      '<input id="phoneNumberInput" type="tel" inputmode="numeric" autocomplete="tel-national" placeholder="' +
      escapeHtml(country.placeholder) +
      '" value="' +
      ...
      "</label>" +
      '<p class="field-note' +
      (validation.valid ? " field-note--success" : state.phoneNumber ? " field-note--error" : "") +
      '">' +
  ...
  function renderOtpScreen() {
    const isManual = state.otpMode === "manual";
    ...
    return (
      ...
      "<p>We sent a secure OTP request to " +
      ...
      (isManual
        ? '<div class="otp-row">' +
          ...
          escapeHtml(state.otpError || "Enter the 6 digit OTP sent to the selected number.") +
          "</p>" +
          '<button class="primary-button primary-button--full primary-button--muted" type="button" data-action="verify-otp"' +
          (isOtpValid() ? "" : " disabled") +
          ">Verify OTP</button>"
        : '<div class="otp-auto-card otp-auto-card--' +
          escapeHtml(state.otpStatus) +
          '">' +
          '<div class="otp-auto-card__dots"><span></span><span></span><span></span><span></span><span></span><span></span></div>' +
          '<strong>' +
          escapeHtml(state.otpStatus === "verified" ? "OTP verified securely" : "Checking SMS automatically") +
          "</strong>" +
          "<p>" +
          escapeHtml(
            state.otpStatus === "verified"
              ? "Your number is confirmed. Moving to the next step."
              : "The OTP stays hidden in this demo and is verified internally by the app."
          ) +
          "</p>" +
          "</div>" +
          '<button class="link-button" type="button" data-action="enter-otp-manually">Enter code manually</button>') +
      '<button class="link-button" type="button" data-action="resend-otp">Resend OTP</button>' +
      "</div>" +
      "</section>"
    );
  }
  ...
  function renderProfilePhotoScreen() {
    const profilePhoto = getProfilePhotoMarkup();

    return (
      '<section class="screen screen--light">' +
      '<div class="auth-card auth-card--profile-photo">' +
      '<div class="auth-card__icon auth-card__icon--profile">' +
      icon("camera") +
      "</div>" +
      "<h1>Add a profile photo</h1>" +
      "<p>Take a photo right now or choose one from your device.</p>" +
      '<div class="profile-photo-preview">' +
      profilePhoto +
      "</div>" +
      '<div class="profile-photo-actions">' +
      '<button class="media-choice" type="button" data-action="capture-profile-photo">' +
      '<span class="media-choice__icon">' +
      icon("camera") +
      "</span>" +
      '<span class="media-choice__copy"><strong>Take photo</strong><small>Open the camera on the device</small></span>' +
      "</button>" +
      '<button class="media-choice" type="button" data-action="upload-profile-photo">' +
      '<span class="media-choice__icon">' +
      icon("image") +
      "</span>" +
      '<span class="media-choice__copy"><strong>Choose from gallery</strong><small>Pick an image from local storage</small></span>' +
      "</button>" +
      "</div>" +
      '<input id="profilePhotoCameraInput" type="file" accept="image/*" capture="user" hidden>' +
      '<input id="profilePhotoUploadInput" type="file" accept="image/*" hidden>' +
      '<div class="auth-actions-stack">' +
      '<button class="primary-button primary-button--full" type="button" data-action="continue-profile-photo">Continue</button>' +
      '<button class="secondary-button secondary-button--full" type="button" data-action="skip-profile-photo">Skip for now</button>' +
      "</div>" +
      "</div>" +
      "</section>"
    );
  ...
  function renderNotificationsScreen() {
    return (
      '<section class="screen screen--light">' +
      '<div class="auth-card auth-card--notifications">' +
      '<div class="auth-card__icon auth-card__icon--otp">' +
      icon("bell") +
      "</div>" +
      "<h1>Enable notifications</h1>" +
      "<p>Get new message alerts, status updates, and missed call reminders.</p>" +
      '<div class="notification-card">' +
      "<strong>How it works</strong>" +
      "<span>We request device notification permission only once and continue even if you skip it.</span>" +
      "</div>" +
      '<div class="auth-actions-stack">' +
      '<button class="primary-button primary-button--full" type="button" data-action="enable-notifications">Enable notifications</button>' +
      '<button class="secondary-button secondary-button--full" type="button" data-action="skip-notifications">Not interested</button>' +
      "</div>" +
      '<p class="field-note">' +
      escapeHtml("Current permission: " + state.notificationPermission) +
      "</p>" +
      "</div>" +
      "</section>"
    );
  ...
  function renderReadyScreen() {
    return (
      ...
      "</p>" +
      '<small class="ready-state__meta">' +
      escapeHtml(
        state.notificationPreference === "enabled"
  ...
  function renderUpdatesScreen() {
    ...
    return (
      ...
      (recentStories.length
        ? '<section class="updates-section"><p class="updates-section__label">Recent status updates</p>' +
          recentStories.map(renderStoryListRow).join("") +
          "</section>"
        : "") +
      (viewedStories.length
        ? '<section class="updates-section"><p class="updates-section__label">Viewed status updates</p>' +
  ...
  function renderChatScreen() {
    ...
    return (
      ...
      "</strong>" +
      '<span class="thread-header__status thread-header__status--' +
      escapeHtml(contact.presence || "offline") +
      '">' +
  ...
  function renderContactRow(contact) {
    return (
      ...
      '<span class="contact-row__title">' +
      "<strong>" +
      escapeHtml(contact.name) +
      "</strong>" +
      renderPresencePill(contact) +
      "</span>" +
      "<small>" +
  ...
  function renderAvatar(contact, className, showPresence) {
    return (
      ...
      (showPresence
        ? '<span class="avatar__presence avatar__presence--' +
          escapeHtml(contact.presence || "offline") +
          '"></span>'
        : "") +
      "</span>"
  ...
  function renderPresencePill(contact) {
    return (
      '<span class="presence-pill presence-pill--' +
      escapeHtml(contact.presence || "offline") +
      '">' +
      escapeHtml(getPresenceLabel(contact)) +
      "</span>"
    );
  }
  ...
  function icon(name) {
    const icons = {
      ...
      bell: '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M6 9.5C6 6.46 8.46 4 11.5 4C14.54 4 17 6.46 17 9.5V13L19 16V17H4V16L6 13V9.5Z"></path><path d="M10 19C10.5 19.7 11 20 11.5 20C12 20 12.5 19.7 13 19"></path></svg>',
      image: '<svg viewBox="0 0 24 24" aria-hidden="true"><rect x="4" y="5" width="16" height="14" rx="2"></rect><circle cx="9" cy="10" r="1.6"></circle><path d="M20 15L15 11L8 18"></path></svg>',
  ...
  function applyRequestedScreen() {
    ...
    if (!state.phoneNumber) {
      state.phoneNumber = getCurrentCountry().placeholder;
    }
  ...
  function startOtpAutoVerification() {
    stopOtpAutoVerification();

    if (!state.generatedOtp) {
      state.generatedOtp = generateOtpCode();
    }

    state.otpStatus = "pending";
    state.otpAutoTimer = window.setTimeout(function () {
      state.otpStatus = "verified";
      render();
      state.otpAutoAdvanceTimer = window.setTimeout(function () {
        navigate("profile");
      }, 420);
    }, otpAutoVerifyDelayMs);
  }

  function stopOtpAutoVerification() {
    window.clearTimeout(state.otpAutoTimer);
    window.clearTimeout(state.otpAutoAdvanceTimer);
    state.otpAutoTimer = null;
    state.otpAutoAdvanceTimer = null;
  }
  ...
  function requestOtp(isResend) {
    ...
    state.otpRequestedFor = getNormalizedPhoneNumber();
    navigate("otp");
    flashToast(isResend ? "A new OTP was requested securely." : "OTP requested for " + getNormalizedPhoneNumber() + ".");
  }

  function verifyOtp() {
    if (state.otpDigits.join("") !== state.generatedOtp) {
      state.otpError = "The OTP is not correct. Please try again or resend it.";
      render({ focusId: "otp-0" });
      return;
    }
  ...
  function openProfilePhotoInput(source) {
    const input = document.getElementById(source === "camera" ? "profilePhotoCameraInput" : "profilePhotoUploadInput");
  ...
  function handleProfilePhotoSelection(file, source) {
    if (!file) {
      return;
    }

    if (!file.type || file.type.indexOf("image/") !== 0) {
      flashToast("Choose an image file for the profile photo.");
      return;
    }
    ...
    reader.onload = function () {
      state.profilePhotoUrl = String(reader.result || "");
      state.profilePhotoSource = source;
      syncOwnStoryProfile();
      flashToast(source === "camera" ? "Profile photo captured." : "Profile photo selected.");
      render();
    };
  ...
  async function enableNotifications() {
    if (typeof window.Notification === "undefined") {
      ...
      state.notificationPreference = "skipped";
      flashToast("Notifications are not supported in this browser.");
      navigate("ready");
      return;
    }

    try {
      const permission = state.notificationPermission === "granted"
        ? "granted"
        : await window.Notification.requestPermission();

      state.notificationPermission = permission;
      state.notificationPreference = permission === "granted" ? "enabled" : "skipped";
      flashToast(
        permission === "granted"
          ? "Notifications enabled."
      ...
      state.notificationPermission = "error";
      state.notificationPreference = "skipped";
      flashToast("Notification permission could not be requested.");
    }
  ...
  function getProfilePhotoMarkup() {
    if (state.profilePhotoUrl) {
      return '<img class="profile-photo-preview__image" src="' + escapeHtml(state.profilePhotoUrl) + '" alt="Profile photo">';
    }

    return (
      '<span class="profile-photo-preview__fallback">' +
      escapeHtml((state.profileName.trim() || "W").slice(0, 1).toUpperCase()) +
      "</span>"
    );
  }

  function syncOwnStoryProfile() {
    const ownStory = getOwnStory();
    ...
    ownStory.avatarSrc = state.profilePhotoUrl || "";
    ownStory.name = state.profileName.trim() ? state.profileName.trim() : "My Status";
    ownStory.title = (state.profileName.trim() || "My") + "'s Status";
  }
  ...
  function loadPersistedState() {
    ...
    try {
      ...
      state.selectedCountryId = saved.selectedCountryId || state.selectedCountryId;
      state.phoneNumber = saved.phoneNumber || state.phoneNumber;
      state.generatedOtp = saved.generatedOtp || state.generatedOtp;
      state.otpMode = saved.otpMode || state.otpMode;
      state.otpStatus = saved.otpStatus || state.otpStatus;
      state.otpRequestedFor = saved.otpRequestedFor || state.otpRequestedFor;
      state.otpRequestedAt = saved.otpRequestedAt || state.otpRequestedAt;
      state.profileName = saved.profileName || state.profileName;
      state.profilePhotoUrl = saved.profilePhotoUrl || state.profilePhotoUrl;
      state.profilePhotoSource = saved.profilePhotoSource || state.profilePhotoSource;
      state.notificationPreference = saved.notificationPreference || state.notificationPreference;
      state.notificationPermission = saved.notificationPermission || state.notificationPermission;
      state.onboardingCompleted = Boolean(saved.onboardingCompleted);
      state.homeTint = saved.homeTint || state.homeTint;
      state.selectedChatId = saved.selectedChatId || state.selectedChatId;
      state.conversations = saved.conversations || state.conversations;
      state.stories = saved.stories || state.stories;
      state.callLogs = saved.callLogs || state.callLogs;
      state.favoriteContactIds = saved.favoriteContactIds || state.favoriteContactIds;
    } catch (error) {
  ...
  function getPhoneValidation() {
    ...
    const regex = new RegExp(country.mobileRegex);

    if (!digits) {
      return {
        valid: false,
        message: "Use a " + country.name + " number like " + country.placeholder + "."
      };
    }
    ...
    if (digits.length < country.minLength) {
      return {
        valid: false,
        message: "Add " + (country.minLength - digits.length) + " more digit(s) for a valid " + country.name + " number."
      };
    }
    ...
    if (digits.length > country.maxLength) {
      return {
        valid: false,
        message: country.name + " mobile numbers can be at most " + country.maxLength + " digits."
      };
    }
    ...
    if (regex.test(digits)) {
      return {
        valid: true,
        message: "This looks like a valid " + country.name + " mobile number."
      };
    }

    const likelyCountry = getLikelyCountryMatch(digits);

    if (likelyCountry && likelyCountry.id !== country.id) {
      return {
        valid: false,
        message: "This number looks closer to a " + likelyCountry.name + " mobile format."
      };
    }

    return {
      valid: false,
      message: "This number does not match the " + country.name + " mobile rules."
    };
  }

  function getLikelyCountryMatch(digits) {
    return data.countries.find(function (country) {
      return new RegExp(country.mobileRegex).test(digits);
    }) || null;
  }

  function getNormalizedPhoneNumber() {
    return getCurrentCountry().dialCode + " " + state.phoneNumber;
  }
  ...
  function getFilteredContacts(query) {
    ...
    return data.contacts.filter(function (contact) {
      return (
        ...
        contact.status.toLowerCase().indexOf(normalized) >= 0 ||
        contact.about.toLowerCase().indexOf(normalized) >= 0
      );
    });
  }
  ...
  function getPresenceLabel(contact) {
    ...
    return labels[contact.presence] || "Offline";
  }

  function getFormattedPhoneNumber() {
    return state.phoneNumber || getCurrentCountry().placeholder;
  }
